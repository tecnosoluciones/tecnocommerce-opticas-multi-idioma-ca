...
if ( ! class_exists( 'WC_SC_Coupon_Process' ) ) {
	...
	class WC_SC_Coupon_Process {
				...
				foreach ( $order_items as $item_id => $item ) {
					...
					if ( is_a( $smart_coupon, 'WC_Coupon' ) ) {
						...
						if ( 'smart_coupon' === $discount_type ) {
							...
							$get_discount_total        = $order_discount_amount + $order_discount_tax_amount;
							$credit_remaining          = round( ( $coupon_amount - $get_discount_total ), get_option( 'woocommerce_price_num_decimals', 2 ) );
		 ...
		 * @return bool|mixed
		 */
		public function virtual_downloadable_item_needs_update_smart_coupon_balance( $virtual_downloadable_item = true, $product = null, $order_id = 0 ) {
			$order                        = wc_get_order( $order_id );
			$hooks_available_for_statuses = array( 'on-hold', 'pending', 'processing', 'completed', 'failed', 'refunded', 'cancelled' );
