...
if ( ! class_exists( 'WC_Smart_Coupons' ) ) {
	...
	class WC_Smart_Coupons {
		 ...
		 *
		 * @var $stored_sc_credit_left
		 */
		private $stored_sc_credit_left = array();
		...
		public function sc_handle_store_credit_application() {
			...
			if ( $this->is_wc_gte_30() && 'yes' === $apply_before_tax ) {
				...
				add_action( 'woocommerce_before_calculate_totals', array( $this, 'reset_sc_credit_left' ), 15 );
				add_action( 'woocommerce_order_after_calculate_totals', array( $this, 'order_calculate_discount_amount' ), 10, 2 );
				add_filter( 'woocommerce_coupon_get_discount_amount', array( $this, 'wc_sc_get_coupon_discount_amount' ), 10, 5 );
			}
		}
		...
		public function smart_coupons_discounted_totals( $total = 0, $cart = null, $cart_contains_subscription = false, $calculation_type = '' ) {
			...
			if ( ! empty( $applied_coupons ) ) {
				foreach ( $applied_coupons as $code ) {
					...
					$discount_type = ( is_object( $coupon ) && is_callable( array( $coupon, 'get_discount_type' ) ) ) ? $coupon->get_discount_type() : '';
					if ( ! empty( $discount ) && 'smart_coupon' === $discount_type ) {
						if ( $this->is_wc_gte_30() ) {
							$coupon_code   = $coupon->get_code();
							$coupon_amount = ( is_object( $coupon ) && is_callable( array( $coupon, 'get_amount' ) ) ) ? $coupon->get_amount() : 0;
						} else {
						...
						if ( $this->is_wc_greater_than( '3.1.2' ) ) {
							...
							$coupon_discount_totals = ( is_object( WC()->cart ) && isset( WC()->cart->coupon_discount_amounts ) ) ? WC()->cart->coupon_discount_amounts : array();
						}
						...
						if ( empty( $coupon_discount_totals[ $coupon_code ] ) ) {
							$coupon_discount_totals[ $coupon_code ] = $coupon_discount_totals[ $coupon_code ] + $total;
						} else {
							$coupon_discount_totals[ $coupon_code ] = $coupon_discount_totals[ $coupon_code ] + $total;
						}

						if ( $coupon_amount >= $coupon_discount_totals[ $coupon_code ] ) {
							$discount = min( $total, $discount );
		...
		public function manage_smart_coupon_credit_used( $coupon = '', $discount = 0, $cart_contains_subscription = false, $calculation_type = '' ) {
			if ( is_object( $coupon ) && $coupon instanceof WC_Coupon ) {
				...
				$cart_total = ( $this->is_wc_greater_than( '3.1.2' ) ) ? WC()->cart->get_total( 'edit' ) : WC()->cart->total;
				if ( $this->is_wc_greater_than( '3.1.2' ) ) {
					$coupon_discount_totals = WC()->cart->get_coupon_discount_totals();
				} else {
					$coupon_discount_totals = ( is_object( WC()->cart ) && isset( WC()->cart->coupon_discount_amounts ) ) ? WC()->cart->coupon_discount_amounts : array();
				...
				if ( empty( $coupon_discount_totals[ $coupon_code ] ) ) {
					$coupon_discount_totals[ $coupon_code ] = $coupon_discount_totals[ $coupon_code ] + $cart_total;
				} else {
					$coupon_discount_totals[ $coupon_code ] = $coupon_discount_totals[ $coupon_code ] + $cart_total;
				}
				...
				if ( empty( $smart_coupon_credit_used[ $coupon_code ] ) || ( $cart_contains_subscription && ( 'combined_total' === $calculation_type || 'sign_up_fee_total' === $calculation_type ) ) ) {
					$smart_coupon_credit_used[ $coupon_code ] = $coupon_discount_totals[ $coupon_code ];
				} else {
					$smart_coupon_credit_used[ $coupon_code ] += $coupon_discount_totals[ $coupon_code ];
				}
		 ...
		 * @param float   $discounting_amount product price.
		 * @param object  $cart_item cart object.
		 * @param boolean $single True if discounting a single qty item, false if its the line.
		 * @param object  $coupon coupon object.
		 * @return float|int|mixed
		 */
		public function wc_sc_get_coupon_discount_amount( $discount, $discounting_amount, $cart_item, $single, $coupon ) {
			$discount_type = ( is_object( $coupon ) && is_callable( array( $coupon, 'get_discount_type' ) ) ) ? $coupon->get_discount_type() : '';
			...
			if ( is_object( $cart_item ) && is_a( $cart_item, 'WC_Order_Item_Product' ) ) {
				$product      = ( is_callable( array( $cart_item, 'get_product' ) ) ) ? $cart_item->get_product() : null;
				...
				$item_id      = ! empty( $cart_item['key'] ) ? $cart_item['key'] : 0;
				// Compatibility for WC version < 3.2.0.
				if ( empty( $item_id ) ) {
					$current_product_id = ( ! empty( $cart_item['variation_id'] ) ) ? $cart_item['variation_id'] : $cart_item['product_id'];
					...
					foreach ( WC()->cart->cart_contents as $key => $cart_data ) {
						...
						if ( $current_product_id === $cart_data_product_id ) {
							$item_id = $key;
						}
					}
				}
			...
			if ( ! empty( $this->stored_sc_credit_left[ $item_id ][ $variation_id ] ) ) {
				return $this->stored_sc_credit_left[ $item_id ][ $variation_id ];
			}if ( ! empty( $this->stored_sc_credit_left[ $item_id ][ $product_id ] ) ) {
				return $this->stored_sc_credit_left[ $item_id ][ $product_id ];
			}

			if ( isset( $this->stored_sc_credit_left[ $coupon_code ] ) && empty( $this->stored_sc_credit_left[ $coupon_code ] ) ) {
				return $discount;
			}

			$credit_left = ( ! empty( $this->stored_sc_credit_left ) && isset( $this->stored_sc_credit_left[ $coupon_code ] ) ) ? $this->stored_sc_credit_left[ $coupon_code ] : $coupon_amount;
			$discount    = $discounting_amount * $quantity;
			if ( ! is_null( $cart_item ) && $credit_left > 0 ) {
				if ( $discount >= $credit_left ) {
					$discount                                    = $credit_left;
					$this->stored_sc_credit_left[ $coupon_code ] = 0;
				} elseif ( $discount < $credit_left ) {
					$credit_left                                 = $credit_left - $discount;
					$this->stored_sc_credit_left[ $coupon_code ] = $credit_left;
				}
			}
			$discount = $discount / $quantity;
			if ( ! empty( $variation_id ) ) {
				$this->stored_sc_credit_left[ $item_id ][ $variation_id ] = $discount;
			} else {
				$this->stored_sc_credit_left[ $item_id ][ $product_id ] = $discount;
			}
			return $discount;
		}
		 ...
		 */
		public function reset_sc_credit_left() {
			$this->stored_sc_credit_left = array();
		}
