...
class WOOF_META_FILTER_TEXTINPUT extends WOOF_META_FILTER_TYPE {
    ...
    public function woof_print_html_type_meta() {
        ...
        if (isset(woof()->settings[$this->meta_key]) AND isset(woof()->settings[$this->meta_key]["show"]) AND woof()->settings[$this->meta_key]["show"]) {
    ...
    public function create_meta_query() {
        $curr_text = str_replace("&#039;", "'", htmlspecialchars_decode($this->check_current_request()));

        if ($curr_text) {
            $compare = 'LIKE';

            if (isset(woof()->settings[$this->meta_key]["text_conditional"]) && woof()->settings[$this->meta_key]["text_conditional"] === '=') {
                $compare = '=';
            }
