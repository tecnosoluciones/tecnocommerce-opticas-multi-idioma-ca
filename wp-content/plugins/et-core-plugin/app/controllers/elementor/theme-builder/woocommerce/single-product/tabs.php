...
class Tabs extends \Elementor\Widget_Base {
	...
	public function get_keywords() {
        return [ 'woocommerce', 'shop', 'store', 'data', 'product', 'tabs', 'toggle' ];
	}
...
//        );
        ...
        $this->add_control(
            ...
            [
                ...
                'selectors' => [
                    '{{WRAPPER}} .wc-tabs .et-woocommerce-tab a' => 'border-color: {{VALUE}};',
                ],
            ]
        );
	...
	public function filter_tabs($tabs) {
        $settings = $this->get_settings_for_display();
        if ( count($settings['tabs_list'] ) )
            return array_unique(array_map(function ($k) {return $k['tab_key'];}, $settings['tabs_list']));
