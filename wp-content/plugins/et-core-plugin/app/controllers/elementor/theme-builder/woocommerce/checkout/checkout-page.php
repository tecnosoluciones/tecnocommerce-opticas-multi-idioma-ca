...
//        );
        ...
        $this->add_control(
            ...
            [
                ...
                'selectors' => [
                    '{{WRAPPER}} .form-row-first, {{WRAPPER}} .form-row-last' => '{{VALUE}}',
                ]
            ]
        );
...
//        $this->end_controls_section();
    ...
    protected function render() {
        ...
        if ( !!$settings['order_review_sticky'] || !!$settings['order_review_product_remove'] )
...
//            remove_filter('wp_doing_ajax', '__return_false');
        ...
        ?>
            <div class="<?php echo implode(' ', apply_filters('etheme_checkout_form_shipping_methods_wrapper_classes', array('woocommerce-shipping-methods-fields'))); ?>">
                <?php if ( apply_filters('etheme_checkout_form_shipping_title', true) ) { ?>
                    <<?php echo apply_filters('etheme_woocommerce_checkout_title_tag', 'h3'); ?> class="<?php echo apply_filters('etheme_woocommerce_checkout_title_class', 'step-title'); ?>">
                    <span><?php echo apply_filters('etheme_woocommerce_checkout_shipping_methods_title', esc_html__( 'Shipping methods', 'xstore' )); ?></span>
                    </<?php echo apply_filters('etheme_woocommerce_checkout_title_tag', 'h3'); ?>>
                <?php }
    ...
    public function get_changable_fields_information() {
        return sprintf(__('You can change %s (optional/required/hidden) properly configured in Theme Options -> WooCommerce -> Checkout Settings.', 'xstore-core'),
            '<a href="'.admin_url( '/customize.php?autofocus[section]=woocommerce_checkout' ).'" target="_blank"><strong>'.esc_html__('Fields statuses', 'xstore-core').'</strong></a>');
    }
