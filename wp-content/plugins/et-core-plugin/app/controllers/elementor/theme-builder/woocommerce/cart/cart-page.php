...
class Cart_Page extends \Elementor\Widget_Base {
    ...
    public function get_style_depends() {
        return ['etheme-cart-page', 'etheme-no-products-found', 'etheme-quantity-types-style', 'etheme-elementor-cart-page'];
    }
    ...
    public function additional_cart_totals_template() {
        ...
        switch ($settings['additional_cart_totals_content_type']) {
            ...
            case 'saved_template':
                ...
                if (!empty($settings[$prefix.$settings['additional_cart_totals_content_type']]) && !in_array($settings[$prefix.$settings['additional_cart_totals_content_type']], array('select', 'no_template'))):
                    //								echo \Elementor\Plugin::$instance->frontend->get_builder_content( $settings[$settings['content_type']], true );
                    $posts = get_posts(
                        [
                            'name' => $settings[$prefix.$settings['additional_cart_totals_content_type']],
                            'post_type'      => 'elementor_library',
                            'posts_per_page' => '1',
                            'tax_query'      => [
                                [
                                    'taxonomy' => 'elementor_library_type',
                                    'field'    => 'slug',
                                    'terms'    => str_replace(array('global_widget', 'saved_template'), array('widget', 'section'), $settings['additional_cart_totals_content_type']),
                                ],
                            ],
                            'fields' => 'ids'
                        ]
