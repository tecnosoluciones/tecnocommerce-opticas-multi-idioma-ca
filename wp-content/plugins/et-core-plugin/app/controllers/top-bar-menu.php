...
//                ));
	    ...
	    if ( isset( $notices['update'] ) ) {
		    ...
		    $wp_admin_bar->add_node( array(
			    ...
			    'title'  => $menu_icons['changelog'] . esc_html__( 'Updates', 'xstore-core' ) . $notice_icon,
...
//			}
        ...
        if ( !$this->settings->hide_theme_builders ) {
            // theme builder
            $this->add_theme_builders_tab($wp_admin_bar, $this->get_theme_builders_menu_icons(), $this->is_woocommerce);
        }

        if ( in_array('sales_booster', $this->settings->show_pages) && $this->is_theme_active && $this->is_woocommerce ) {
            $wp_admin_bar->add_node(array(
    ...
    public function add_theme_builders_tab($wp_admin_bar, $menu_icons, $is_woocommerce) {
        ...
        $builder_url = ( $this->is_elementor ) ? $elementor_builders_url : admin_url('/customize.php?autofocus[panel]=header-builder');
        $args = array(
...
//            'title' => '<span class="ab-label"><img class="et-logo" style="vertical-align: -4px; margin-inline-end: 5px; max-width: 18px;" src="' . $this->settings->title_logo . '" alt="xstore"><span>' . esc_html__('Theme Builders', 'xstore-core') . '</span>',
        ...
        if ( $this->is_elementor ) {
            ...
            $elementor_pro_theme_builder_link = $this->is_elementor ? \Elementor\Plugin::$instance->app->get_settings('menu_url') : $builder_url;
            ...
            if ( $is_woocommerce ) {
                ...
                if ( !$this->is_elementor || get_option( 'etheme_single_product_builder', false ) ) {
                    $theme_builders = array_merge($theme_builders, array(
                        'product-old' => esc_html__('Single Product', 'xstore-core') . ($this->is_elementor ? ' ' . esc_html__('(old)', 'xstore-core') : ''),
                    ));
                }
                $theme_builders = array_merge($theme_builders, array(
            ...
            foreach ($theme_builders as $theme_builder_unique_key => $theme_builder_unique_name) {
                ...
                if ( !$this->is_elementor && !in_array($theme_builder_unique_key, array('header', 'product-old'))) continue;

                $builder_url = $this->is_elementor && !$has_pro ? $elementor_builders_url.'&et_trigger_theme_builders_plugins_popup=true' : $elementor_pro_theme_builder_link . '/templates/'.$theme_builder_unique_key;
                switch ($theme_builder_unique_key) {
                    case 'cart':
                    case 'checkout':
                        if ( $has_pro ) {
                            if ( !get_option('et_theme_builder_wc_install_done', false) && method_exists('\WC_Install', 'create_pages') ) {
                    ...
                    case 'header':
                        ...
                        break;
                    case 'product-old':
                        $builder_url = admin_url('/customize.php?autofocus[panel]=single_product_builder');
                    break;
                }
                $wp_admin_bar->add_node(array(
                    'parent' => 'et-top-bar-theme-builders-menu',
