...
//				$this->page['class'] = 'ai';
    ...
    public function et_add_menu_page(){
        ...
        $priorities = array(
            'welcome' => 52,
            'theme-builders' => 52.2,
            'static-blocks' => 54, // in another file
            'portfolio' => 55, // in another file
            'sales-booster' => 52.3,
            'widgets' => 52.35,
            'email-builder' => 52.4,
            'theme-options' => 52.1
        ...
        $icons = array(
            ...
            'static-blocks' => 'dashicons-welcome-widgets-menus', // in another file
            'portfolio' => 'dashicons-images-alt2',
...
//            }
        ...
        if ( $is_activated && in_array('custom_fonts', $show_pages) ) {
            add_submenu_page(
                'et-panel-welcome',
        ...
        if ( $is_activated && $is_et_core ) {
            ...
            if ( !class_exists('XStore_AMP') ) {
                add_submenu_page(
                    'et-panel-welcome',
                    sprintf(esc_html__('AMP %s', 'xstore'), $plugin_label),
                    sprintf(esc_html__('AMP %s', 'xstore'), $plugin_label),
                    'manage_options',
                    'et-panel-plugins',
                    array($this, 'etheme_panel_page')
...
//	        }
            ...
            if ( $is_wc && in_array( 'email_builder', $show_pages ) ) {
                add_menu_page(
            ...
            if ( $is_et_core && $is_wc && in_array( 'sales_booster', $show_pages ) ) {
                add_menu_page(
            ...
            if ( $is_activated && $is_et_core && !$hide_theme_builders && class_exists('\Elementor\Plugin') ) {
...
//            $theme_builder_key = 'et-elementor'. ($has_pro ? '-pro' : '') . '-theme-builder';
                ...
                add_submenu_page(
                    ...
                    $theme_builders = array(
                        ...
                        'footer' => esc_html__('Footer', 'xstore'),
                    );
                    if ( $is_wc ) {
                        $theme_builders = array_merge($theme_builders,
                            array(
                                'product' => esc_html__('Single Product', 'xstore'),
                                'product-archive' => esc_html__('Products Archive', 'xstore'),
                                'cart' => esc_html__('Cart Page', 'xstore'),
                                'checkout' => esc_html__('Checkout Page', 'xstore'),
                            ));
                    }
                    $theme_builders = array_merge($theme_builders, array(
                        'error-404' => esc_html__('Error-404', 'xstore'),
                    ));
            ...
            if ( class_exists('Kirki') && in_array('customize', $show_pages) ) {
                if ( $is_activated && $is_et_core ) {
                    add_menu_page(
...
//			$local_settings_parsed[ $local_settings_key ][ $setting['name'] ] = $setting['value'];
    ...
    public function get_filters_form($filters_list = array(), $extra_settings = array()) {
        ...
        <ul class="et-filters et-filters-type-1">
            <li>
                <span class="et-filter-toggle<?php if (!$settings['title']) echo ' et-filter-toggle-icon'; ?>">
                    ...
                    <?php
                        foreach ($filters_list as $filter_key => $filter_details) {
                            ...
                            <li class="et-filter <?php echo esc_attr($settings['type']); if ( $active_key && $filter_key == $active_key ) echo ' active'; ?>" data-filter="<?php echo esc_attr($filter_key); ?>">
                                <?php if ( $filter_link ) { ?>
                                    <a href="<?php echo esc_url($filter_link); ?>" target="_blank" rel="nofollow">
                                <?php } ?>
                                    <span><?php echo esc_html($filter_title); ?></span>
                                    <?php
                                    if ( $settings['arrow']) : ?>
                                        <svg width="0.85em" height="0.85em" viewBox="0 0 10 10" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    public function get_additional_panel_blocks($type = '') {
        ...
        $xstore_branding_settings = get_option( 'xstore_white_label_branding_settings', array() );
        $xstore_branding_settings_default = true;
        $settings = array();
        ...
        if ( count( $xstore_branding_settings ) ) {
            ...
            $xstore_branding_settings_default = false;
        }
        ?>
        <div class="xstore-panel-info-blocks">
            <div class="xstore-panel-info-block">
                ...
                <div>
                    ...
                    <p><a href="<?php if ($xstore_branding_settings_default) echo 'https://www.8theme.com/contact-us/'; else echo esc_url($settings['support_url']); ?>" target="_blank" rel="nofollow"><?php echo esc_html__('Send a message', 'xstore'); ?></a></p>
                </div>
            </div>
            <div class="xstore-panel-info-block">
                ...
                <div>
                    ...
                    <p><a href="<?php echo esc_url($settings['support_url']); ?>" target="_blank" rel="nofollow"><?php echo esc_html__('Get assistance now', 'xstore'); ?></a></p>
                </div>
            </div>
        </div>
        <?php
