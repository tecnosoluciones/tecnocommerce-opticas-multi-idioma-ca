...
if(is_shop() ){
    echo do_shortcode('[elementor-template id="8285"]');
}
else if(is_product_category()){
    echo do_shortcode('[elementor-template id="9063"]');
}
else if(is_product_taxonomy()){
    echo do_shortcode('[elementor-template id="9513"]');
}
else if($wp_query->query['pagename'] == 'blog'){
    echo do_shortcode('[elementor-template id="7482"]');
}
else if($wp_query->query['pagename'] == 'inicio'){

}
else{
    if(count($wp_query->query) != 0 && !is_single()){
       echo do_shortcode('[elementor-template id="7507"]');
    }
}